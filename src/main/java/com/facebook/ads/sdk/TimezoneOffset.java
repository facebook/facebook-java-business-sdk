...
public class TimezoneOffset extends APINode {
  ...
  public static TimezoneOffset loadJSON(String json, APIContext context, String header) {
    ...
    if (context.isDebug()) {
      ...
      JsonElement o2 = JsonParser.parseString(timezoneOffset.toString());
  ...
  public static APINodeList<TimezoneOffset> parseResponse(String json, APIContext context, APIRequest request, String header) throws MalformedResponseException {
    ...
    try{
      ...
      if (result.isJsonArray()) {
        ...
        for (int i = 0; i < arr.size(); i++) {
          timezoneOffsets.add(loadJSON(arr.get(i).getAsJsonObject().toString(), context, header));
        };
        ...
        if (obj.has("data")) {
            ...
            for (String s : new String[]{"campaigns", "adsets", "ads"}) {
              if (obj.has(s)) {
                ...
                for (Map.Entry<String, JsonElement> entry : obj.entrySet()) {
                  timezoneOffsets.add(loadJSON(entry.getValue().toString(), context, header));
                }
                break;
              }
            }
            if (!isRedownload) {
              timezoneOffsets.add(loadJSON(obj.toString(), context, header));
            }
          }
          return timezoneOffsets;
          ...
          for (Map.Entry entry : obj.entrySet()) {
            ...
            if (
              ...
              timezoneOffsets.add(loadJSON(value.toString(), context, header));
            } else {
          ...
          timezoneOffsets.add(loadJSON(json, context, header));
          return timezoneOffsets;
        }
      }
    } catch (Exception e) {
  ...
  public static APIRequest.ResponseParser<TimezoneOffset> getParser() {
    return new APIRequest.ResponseParser<TimezoneOffset>() {
      public APINodeList<TimezoneOffset> parseResponse(String response, APIContext context, APIRequest<TimezoneOffset> request, String header) throws MalformedResponseException {
