...
public class PagePublisher extends APINode {
  ...
  public static PagePublisher loadJSON(String json, APIContext context, String header) {
    ...
    if (context.isDebug()) {
      ...
      JsonElement o2 = JsonParser.parseString(pagePublisher.toString());
        ...
        if (obj.has("data")) {
          if (obj.has("paging")) {
            ...
            if (paging.has("cursors")) {
                ...
                pagePublishers.setCursors(before, after);
            }
            String previous = paging.has("previous") ? paging.get("previous").getAsString() : null;
            String next = paging.has("next") ? paging.get("next").getAsString() : null;
            pagePublishers.setPaging(previous, next);
            if (context.hasAppSecret()) {
              pagePublishers.setAppSecret(context.getAppSecretProof());
            }
          }
