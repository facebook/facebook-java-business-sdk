...
public class AdCampaignGroupMetricsMetadata extends APINode {
  ...
  public static AdCampaignGroupMetricsMetadata loadJSON(String json, APIContext context) {
  ...
  public static APINodeList<AdCampaignGroupMetricsMetadata> parseResponse(String json, APIContext context, APIRequest request) throws MalformedResponseException {
    APINodeList<AdCampaignGroupMetricsMetadata> adCampaignGroupMetricsMetadatas = new APINodeList<AdCampaignGroupMetricsMetadata>(request, json);
    JsonArray arr;
    JsonObject obj;
    JsonParser parser = new JsonParser();
    ...
    try{
      ...
      if (result.isJsonArray()) {
        ...
        for (int i = 0; i < arr.size(); i++) {
          adCampaignGroupMetricsMetadatas.add(loadJSON(arr.get(i).getAsJsonObject().toString(), context));
        };
        ...
        if (obj.has("data")) {
            ...
            for (String s : new String[]{"campaigns", "adsets", "ads"}) {
              if (obj.has(s)) {
                ...
                for (Map.Entry<String, JsonElement> entry : obj.entrySet()) {
                  adCampaignGroupMetricsMetadatas.add(loadJSON(entry.getValue().toString(), context));
                }
                break;
              }
            }
            if (!isRedownload) {
              adCampaignGroupMetricsMetadatas.add(loadJSON(obj.toString(), context));
            }
          }
          return adCampaignGroupMetricsMetadatas;
          ...
          for (Map.Entry entry : obj.entrySet()) {
            ...
            if (
              ...
              adCampaignGroupMetricsMetadatas.add(loadJSON(value.toString(), context));
            } else {
          ...
          adCampaignGroupMetricsMetadatas.add(loadJSON(json, context));
          return adCampaignGroupMetricsMetadatas;
        }
      }
    } catch (Exception e) {
  ...
  public static APIRequest.ResponseParser<AdCampaignGroupMetricsMetadata> getParser() {
    return new APIRequest.ResponseParser<AdCampaignGroupMetricsMetadata>() {
      public APINodeList<AdCampaignGroupMetricsMetadata> parseResponse(String response, APIContext context, APIRequest<AdCampaignGroupMetricsMetadata> request) throws MalformedResponseException {
        return AdCampaignGroupMetricsMetadata.parseResponse(response, context, request);
      }
    };
  }
}
