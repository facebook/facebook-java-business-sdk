...
public class PhotoMetadata extends APINode {
  ...
  public static PhotoMetadata loadJSON(String json, APIContext context, String header) {
    PhotoMetadata photoMetadata = getGson().fromJson(json, PhotoMetadata.class);
    if (context.isDebug()) {
      JsonElement o1 = JsonParser.parseString(json);
      JsonElement o2 = JsonParser.parseString(photoMetadata.toString());
  ...
  public static APINodeList<PhotoMetadata> parseResponse(String json, APIContext context, APIRequest request, String header) throws MalformedResponseException {
    ...
    try{
      ...
      if (result.isJsonArray()) {
        ...
        for (int i = 0; i < arr.size(); i++) {
          photoMetadatas.add(loadJSON(arr.get(i).getAsJsonObject().toString(), context, header));
        };
        ...
        if (obj.has("data")) {
          if (obj.has("paging")) {
            ...
            photoMetadatas.setPaging(previous, next);
            ...
            for (String s : new String[]{"campaigns", "adsets", "ads"}) {
              if (obj.has(s)) {
                ...
                for (Map.Entry<String, JsonElement> entry : obj.entrySet()) {
                  photoMetadatas.add(loadJSON(entry.getValue().toString(), context, header));
                }
                break;
              }
            }
            if (!isRedownload) {
              photoMetadatas.add(loadJSON(obj.toString(), context, header));
            }
          }
          return photoMetadatas;
          ...
          for (Map.Entry entry : obj.entrySet()) {
            ...
            if (
              ...
              photoMetadatas.add(loadJSON(value.toString(), context, header));
            } else {
          ...
          photoMetadatas.add(loadJSON(json, context, header));
          return photoMetadatas;
        }
      }
    } catch (Exception e) {
  ...
  public static APIRequest.ResponseParser<PhotoMetadata> getParser() {
    return new APIRequest.ResponseParser<PhotoMetadata>() {
      public APINodeList<PhotoMetadata> parseResponse(String response, APIContext context, APIRequest<PhotoMetadata> request, String header) throws MalformedResponseException {
