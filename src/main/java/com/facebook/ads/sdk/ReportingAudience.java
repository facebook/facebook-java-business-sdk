...
public class ReportingAudience extends APINode {
  ...
  @SerializedName("custom_audiences_url_param_name")
  private String mCustomAudiencesUrlParamName = null;
  @SerializedName("custom_audiences_url_param_type")
  private String mCustomAudiencesUrlParamType = null;
  protected static Gson gson = null;

  public ReportingAudience() {
  }
  ...
  public static ReportingAudience loadJSON(String json, APIContext context, String header) {
    ReportingAudience reportingAudience = getGson().fromJson(json, ReportingAudience.class);
    if (context.isDebug()) {
      JsonElement o1 = JsonParser.parseString(json);
      JsonElement o2 = JsonParser.parseString(reportingAudience.toString());
  ...
  public static APINodeList<ReportingAudience> parseResponse(String json, APIContext context, APIRequest request, String header) throws MalformedResponseException {
    ...
    try{
      ...
      if (result.isJsonArray()) {
        ...
        for (int i = 0; i < arr.size(); i++) {
          reportingAudiences.add(loadJSON(arr.get(i).getAsJsonObject().toString(), context, header));
        };
        ...
        if (obj.has("data")) {
          if (obj.has("paging")) {
            ...
            reportingAudiences.setPaging(previous, next);
            ...
            for (String s : new String[]{"campaigns", "adsets", "ads"}) {
              if (obj.has(s)) {
                ...
                for (Map.Entry<String, JsonElement> entry : obj.entrySet()) {
                  reportingAudiences.add(loadJSON(entry.getValue().toString(), context, header));
                }
                break;
              }
            }
            if (!isRedownload) {
              reportingAudiences.add(loadJSON(obj.toString(), context, header));
            }
          }
          return reportingAudiences;
          ...
          for (Map.Entry entry : obj.entrySet()) {
            ...
            if (
              ...
              reportingAudiences.add(loadJSON(value.toString(), context, header));
            } else {
          ...
          reportingAudiences.add(loadJSON(json, context, header));
          return reportingAudiences;
        }
      }
    } catch (Exception e) {
  ...
  public String getFieldCustomAudiencesUrlParamName() {
    return mCustomAudiencesUrlParamName;
  }
  ...
  public String getFieldCustomAudiencesUrlParamType() {
    return mCustomAudiencesUrlParamType;
  }
  ...
  public static APIRequest.ResponseParser<ReportingAudience> getParser() {
    return new APIRequest.ResponseParser<ReportingAudience>() {
      public APINodeList<ReportingAudience> parseResponse(String response, APIContext context, APIRequest<ReportingAudience> request, String header) throws MalformedResponseException {
