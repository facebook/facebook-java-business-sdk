...
public class VideoUploadLimits extends APINode {
  ...
  public static VideoUploadLimits loadJSON(String json, APIContext context, String header) {
    ...
    videoUploadLimits.header = header;
    return videoUploadLimits;
  }

  public static APINodeList<VideoUploadLimits> parseResponse(String json, APIContext context, APIRequest request, String header) throws MalformedResponseException {
    ...
    try{
      ...
      if (result.isJsonArray()) {
        ...
        for (int i = 0; i < arr.size(); i++) {
          videoUploadLimitss.add(loadJSON(arr.get(i).getAsJsonObject().toString(), context, header));
        };
        ...
        if (obj.has("data")) {
            ...
            for (String s : new String[]{"campaigns", "adsets", "ads"}) {
              if (obj.has(s)) {
                ...
                for (Map.Entry<String, JsonElement> entry : obj.entrySet()) {
                  videoUploadLimitss.add(loadJSON(entry.getValue().toString(), context, header));
                }
                break;
              }
            }
            if (!isRedownload) {
              videoUploadLimitss.add(loadJSON(obj.toString(), context, header));
            }
          }
          return videoUploadLimitss;
          ...
          for (Map.Entry entry : obj.entrySet()) {
            ...
            if (
              ...
              videoUploadLimitss.add(loadJSON(value.toString(), context, header));
            } else {
          ...
          videoUploadLimitss.add(loadJSON(json, context, header));
          return videoUploadLimitss;
        }
      }
    } catch (Exception e) {
  ...
  public static APIRequest.ResponseParser<VideoUploadLimits> getParser() {
    return new APIRequest.ResponseParser<VideoUploadLimits>() {
      public APINodeList<VideoUploadLimits> parseResponse(String response, APIContext context, APIRequest<VideoUploadLimits> request, String header) throws MalformedResponseException {
        return VideoUploadLimits.parseResponse(response, context, request, header);
      }
    };
  }
}
