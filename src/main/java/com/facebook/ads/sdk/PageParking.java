...
public class PageParking extends APINode {
  ...
  public static PageParking loadJSON(String json, APIContext context, String header) {
    ...
    if (context.isDebug()) {
      ...
      JsonElement o2 = JsonParser.parseString(pageParking.toString());
        ...
        if (obj.has("data")) {
          if (obj.has("paging")) {
            ...
            if (paging.has("cursors")) {
                ...
                pageParkings.setCursors(before, after);
            }
            String previous = paging.has("previous") ? paging.get("previous").getAsString() : null;
            String next = paging.has("next") ? paging.get("next").getAsString() : null;
            pageParkings.setPaging(previous, next);
            if (context.hasAppSecret()) {
              pageParkings.setAppSecret(context.getAppSecretProof());
            }
          }
