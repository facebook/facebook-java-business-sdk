...
public class OpenGraphRating extends APINode {
  ...
  public static OpenGraphRating loadJSON(String json, APIContext context) {
    OpenGraphRating openGraphRating = getGson().fromJson(json, OpenGraphRating.class);
    if (context.isDebug()) {
      JsonParser parser = new JsonParser();
      JsonElement o1 = parser.parse(json);
      JsonElement o2 = parser.parse(openGraphRating.toString());
    ...
    openGraphRating.context = context;
    openGraphRating.rawValue = json;
    return openGraphRating;
  }

  public static APINodeList<OpenGraphRating> parseResponse(String json, APIContext context, APIRequest request) throws MalformedResponseException {
    APINodeList<OpenGraphRating> openGraphRatings = new APINodeList<OpenGraphRating>(request, json);
    JsonArray arr;
    JsonObject obj;
    JsonParser parser = new JsonParser();
    ...
    try{
      ...
      if (result.isJsonArray()) {
        ...
        for (int i = 0; i < arr.size(); i++) {
          openGraphRatings.add(loadJSON(arr.get(i).getAsJsonObject().toString(), context));
        };
        return openGraphRatings;
        ...
        if (obj.has("data")) {
          if (obj.has("paging")) {
            ...
            if (paging.has("cursors")) {
                ...
                openGraphRatings.setCursors(before, after);
            }
            String previous = paging.has("previous") ? paging.get("previous").getAsString() : null;
            String next = paging.has("next") ? paging.get("next").getAsString() : null;
            openGraphRatings.setPaging(previous, next);
            if (context.hasAppSecret()) {
              openGraphRatings.setAppSecret(context.getAppSecretProof());
            }
          }
            ...
            for (String s : new String[]{"campaigns", "adsets", "ads"}) {
              if (obj.has(s)) {
                ...
                for (Map.Entry<String, JsonElement> entry : obj.entrySet()) {
                  openGraphRatings.add(loadJSON(entry.getValue().toString(), context));
                }
                break;
              }
            }
            if (!isRedownload) {
              openGraphRatings.add(loadJSON(obj.toString(), context));
            }
          }
          return openGraphRatings;
          ...
          for (Map.Entry entry : obj.entrySet()) {
            ...
            if (
              ...
              openGraphRatings.add(loadJSON(value.toString(), context));
            } else {
          ...
          openGraphRatings.clear();
          openGraphRatings.add(loadJSON(json, context));
          return openGraphRatings;
        }
      }
    } catch (Exception e) {
  ...
  public static APIRequest.ResponseParser<OpenGraphRating> getParser() {
    return new APIRequest.ResponseParser<OpenGraphRating>() {
      public APINodeList<OpenGraphRating> parseResponse(String response, APIContext context, APIRequest<OpenGraphRating> request) throws MalformedResponseException {
        return OpenGraphRating.parseResponse(response, context, request);
      }
    };
  }
}
