...
public class AdBidAdjustments extends APINode {
  ...
  public static AdBidAdjustments loadJSON(String json, APIContext context, String header) {
    ...
    if (context.isDebug()) {
      ...
      JsonElement o2 = JsonParser.parseString(adBidAdjustments.toString());
    ...
    adBidAdjustments.header = header;
    return adBidAdjustments;
  }

  public static APINodeList<AdBidAdjustments> parseResponse(String json, APIContext context, APIRequest request, String header) throws MalformedResponseException {
    APINodeList<AdBidAdjustments> adBidAdjustmentss = new APINodeList<AdBidAdjustments>(request, json, header);
    JsonArray arr;
    JsonObject obj;
    ...
    try{
      ...
      if (result.isJsonArray()) {
        ...
        for (int i = 0; i < arr.size(); i++) {
          adBidAdjustmentss.add(loadJSON(arr.get(i).getAsJsonObject().toString(), context, header));
        };
        ...
        if (obj.has("data")) {
            ...
            for (String s : new String[]{"campaigns", "adsets", "ads"}) {
              if (obj.has(s)) {
                ...
                for (Map.Entry<String, JsonElement> entry : obj.entrySet()) {
                  adBidAdjustmentss.add(loadJSON(entry.getValue().toString(), context, header));
                }
                break;
              }
            }
            if (!isRedownload) {
              adBidAdjustmentss.add(loadJSON(obj.toString(), context, header));
            }
          }
          return adBidAdjustmentss;
          ...
          for (Map.Entry entry : obj.entrySet()) {
            ...
            if (
              ...
              adBidAdjustmentss.add(loadJSON(value.toString(), context, header));
            } else {
          ...
          adBidAdjustmentss.add(loadJSON(json, context, header));
          return adBidAdjustmentss;
        }
      }
    } catch (Exception e) {
  ...
  public static APIRequest.ResponseParser<AdBidAdjustments> getParser() {
    return new APIRequest.ResponseParser<AdBidAdjustments>() {
      public APINodeList<AdBidAdjustments> parseResponse(String response, APIContext context, APIRequest<AdBidAdjustments> request, String header) throws MalformedResponseException {
        return AdBidAdjustments.parseResponse(response, context, request, header);
      }
    };
  }
}
