...
public class RateCard extends APINode {
  ...
  public static RateCard loadJSON(String json, APIContext context) {
    RateCard rateCard = getGson().fromJson(json, RateCard.class);
    if (context.isDebug()) {
  ...
  public static APINodeList<RateCard> parseResponse(String json, APIContext context, APIRequest request) throws MalformedResponseException {
    ...
    try{
      ...
      if (result.isJsonArray()) {
        ...
        for (int i = 0; i < arr.size(); i++) {
          rateCards.add(loadJSON(arr.get(i).getAsJsonObject().toString(), context));
        };
        return rateCards;
        ...
        if (obj.has("data")) {
          if (obj.has("paging")) {
            ...
            rateCards.setPaging(before, after);
          }
            ...
            for (String s : new String[]{"campaigns", "adsets", "ads"}) {
              if (obj.has(s)) {
                ...
                for (Map.Entry<String, JsonElement> entry : obj.entrySet()) {
                  rateCards.add(loadJSON(entry.getValue().toString(), context));
                }
                break;
              }
            }
            if (!isRedownload) {
              rateCards.add(loadJSON(obj.toString(), context));
            }
          }
          return rateCards;
          ...
          for (Map.Entry entry : obj.entrySet()) {
            ...
            if (
              ...
              rateCards.add(loadJSON(value.toString(), context));
            } else {
          ...
          rateCards.clear();
          rateCards.add(loadJSON(json, context));
          return rateCards;
        }
      }
    } catch (Exception e) {
  ...
  public static APIRequest.ResponseParser<RateCard> getParser() {
    return new APIRequest.ResponseParser<RateCard>() {
      public APINodeList<RateCard> parseResponse(String response, APIContext context, APIRequest<RateCard> request) throws MalformedResponseException {
