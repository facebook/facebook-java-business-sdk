...
public class AdCreativeCollectionThumbnailInfo extends APINode {
  ...
  public static AdCreativeCollectionThumbnailInfo loadJSON(String json, APIContext context, String header) {
    ...
    if (context.isDebug()) {
      ...
      JsonElement o2 = JsonParser.parseString(adCreativeCollectionThumbnailInfo.toString());
    ...
    adCreativeCollectionThumbnailInfo.header = header;
    return adCreativeCollectionThumbnailInfo;
  }

  public static APINodeList<AdCreativeCollectionThumbnailInfo> parseResponse(String json, APIContext context, APIRequest request, String header) throws MalformedResponseException {
    APINodeList<AdCreativeCollectionThumbnailInfo> adCreativeCollectionThumbnailInfos = new APINodeList<AdCreativeCollectionThumbnailInfo>(request, json, header);
    JsonArray arr;
    JsonObject obj;
    ...
    try{
      ...
      if (result.isJsonArray()) {
        ...
        for (int i = 0; i < arr.size(); i++) {
          adCreativeCollectionThumbnailInfos.add(loadJSON(arr.get(i).getAsJsonObject().toString(), context, header));
        };
        ...
        if (obj.has("data")) {
            ...
            for (String s : new String[]{"campaigns", "adsets", "ads"}) {
              if (obj.has(s)) {
                ...
                for (Map.Entry<String, JsonElement> entry : obj.entrySet()) {
                  adCreativeCollectionThumbnailInfos.add(loadJSON(entry.getValue().toString(), context, header));
                }
                break;
              }
            }
            if (!isRedownload) {
              adCreativeCollectionThumbnailInfos.add(loadJSON(obj.toString(), context, header));
            }
          }
          return adCreativeCollectionThumbnailInfos;
          ...
          for (Map.Entry entry : obj.entrySet()) {
            ...
            if (
              ...
              adCreativeCollectionThumbnailInfos.add(loadJSON(value.toString(), context, header));
            } else {
          ...
          adCreativeCollectionThumbnailInfos.add(loadJSON(json, context, header));
          return adCreativeCollectionThumbnailInfos;
        }
      }
    } catch (Exception e) {
  ...
  public static APIRequest.ResponseParser<AdCreativeCollectionThumbnailInfo> getParser() {
    return new APIRequest.ResponseParser<AdCreativeCollectionThumbnailInfo>() {
      public APINodeList<AdCreativeCollectionThumbnailInfo> parseResponse(String response, APIContext context, APIRequest<AdCreativeCollectionThumbnailInfo> request, String header) throws MalformedResponseException {
        return AdCreativeCollectionThumbnailInfo.parseResponse(response, context, request, header);
      }
    };
  }
}
