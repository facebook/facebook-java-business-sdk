...
public class EngagementInsightsRule extends APINode {
  ...
  public static EngagementInsightsRule loadJSON(String json, APIContext context, String header) {
    EngagementInsightsRule engagementInsightsRule = getGson().fromJson(json, EngagementInsightsRule.class);
    if (context.isDebug()) {
    ...
    engagementInsightsRule.header = header;
    return engagementInsightsRule;
  }

  public static APINodeList<EngagementInsightsRule> parseResponse(String json, APIContext context, APIRequest request, String header) throws MalformedResponseException {
    APINodeList<EngagementInsightsRule> engagementInsightsRules = new APINodeList<EngagementInsightsRule>(request, json, header);
    JsonArray arr;
    JsonObject obj;
    JsonParser parser = new JsonParser();
    ...
    try{
      ...
      if (result.isJsonArray()) {
        ...
        for (int i = 0; i < arr.size(); i++) {
          engagementInsightsRules.add(loadJSON(arr.get(i).getAsJsonObject().toString(), context, header));
        };
        ...
        if (obj.has("data")) {
          if (obj.has("paging")) {
            ...
            engagementInsightsRules.setPaging(previous, next);
            ...
            for (String s : new String[]{"campaigns", "adsets", "ads"}) {
              if (obj.has(s)) {
                ...
                for (Map.Entry<String, JsonElement> entry : obj.entrySet()) {
                  engagementInsightsRules.add(loadJSON(entry.getValue().toString(), context, header));
                }
                break;
              }
            }
            if (!isRedownload) {
              engagementInsightsRules.add(loadJSON(obj.toString(), context, header));
            }
          }
          return engagementInsightsRules;
          ...
          for (Map.Entry entry : obj.entrySet()) {
            ...
            if (
              ...
              engagementInsightsRules.add(loadJSON(value.toString(), context, header));
            } else {
          ...
          engagementInsightsRules.add(loadJSON(json, context, header));
          return engagementInsightsRules;
        }
      }
    } catch (Exception e) {
  ...
  public static APIRequest.ResponseParser<EngagementInsightsRule> getParser() {
    return new APIRequest.ResponseParser<EngagementInsightsRule>() {
      public APINodeList<EngagementInsightsRule> parseResponse(String response, APIContext context, APIRequest<EngagementInsightsRule> request, String header) throws MalformedResponseException {
        return EngagementInsightsRule.parseResponse(response, context, request, header);
      }
    };
  }
}
