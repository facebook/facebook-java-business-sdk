...
public class AdRuleExecutionOptions extends APINode {
  ...
  public static AdRuleExecutionOptions loadJSON(String json, APIContext context, String header) {
    ...
    if (context.isDebug()) {
      ...
      JsonElement o2 = JsonParser.parseString(adRuleExecutionOptions.toString());
    ...
    adRuleExecutionOptions.header = header;
    return adRuleExecutionOptions;
  }

  public static APINodeList<AdRuleExecutionOptions> parseResponse(String json, APIContext context, APIRequest request, String header) throws MalformedResponseException {
    APINodeList<AdRuleExecutionOptions> adRuleExecutionOptionss = new APINodeList<AdRuleExecutionOptions>(request, json, header);
    JsonArray arr;
    JsonObject obj;
    ...
    try{
      ...
      if (result.isJsonArray()) {
        ...
        for (int i = 0; i < arr.size(); i++) {
          adRuleExecutionOptionss.add(loadJSON(arr.get(i).getAsJsonObject().toString(), context, header));
        };
        ...
        if (obj.has("data")) {
            ...
            for (String s : new String[]{"campaigns", "adsets", "ads"}) {
              if (obj.has(s)) {
                ...
                for (Map.Entry<String, JsonElement> entry : obj.entrySet()) {
                  adRuleExecutionOptionss.add(loadJSON(entry.getValue().toString(), context, header));
                }
                break;
              }
            }
            if (!isRedownload) {
              adRuleExecutionOptionss.add(loadJSON(obj.toString(), context, header));
            }
          }
          return adRuleExecutionOptionss;
          ...
          for (Map.Entry entry : obj.entrySet()) {
            ...
            if (
              ...
              adRuleExecutionOptionss.add(loadJSON(value.toString(), context, header));
            } else {
          ...
          adRuleExecutionOptionss.add(loadJSON(json, context, header));
          return adRuleExecutionOptionss;
        }
      }
    } catch (Exception e) {
  ...
  public static APIRequest.ResponseParser<AdRuleExecutionOptions> getParser() {
    return new APIRequest.ResponseParser<AdRuleExecutionOptions>() {
      public APINodeList<AdRuleExecutionOptions> parseResponse(String response, APIContext context, APIRequest<AdRuleExecutionOptions> request, String header) throws MalformedResponseException {
        return AdRuleExecutionOptions.parseResponse(response, context, request, header);
      }
    };
  }
}
