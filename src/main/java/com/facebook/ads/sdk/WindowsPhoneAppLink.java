...
public class WindowsPhoneAppLink extends APINode {
  ...
  public static WindowsPhoneAppLink loadJSON(String json, APIContext context, String header) {
    ...
    if (context.isDebug()) {
      ...
      JsonElement o2 = JsonParser.parseString(windowsPhoneAppLink.toString());
        ...
        if (obj.has("data")) {
          if (obj.has("paging")) {
            ...
            if (paging.has("cursors")) {
                ...
                windowsPhoneAppLinks.setCursors(before, after);
            }
            String previous = paging.has("previous") ? paging.get("previous").getAsString() : null;
            String next = paging.has("next") ? paging.get("next").getAsString() : null;
            windowsPhoneAppLinks.setPaging(previous, next);
            if (context.hasAppSecret()) {
              windowsPhoneAppLinks.setAppSecret(context.getAppSecretProof());
            }
          }
