...
public class DayPart extends APINode {
  ...
  public static DayPart loadJSON(String json, APIContext context, String header) {
    ...
    if (context.isDebug()) {
      ...
      JsonElement o2 = JsonParser.parseString(dayPart.toString());
        ...
        if (obj.has("data")) {
          if (obj.has("paging")) {
            ...
            if (paging.has("cursors")) {
                ...
                dayParts.setCursors(before, after);
            }
            String previous = paging.has("previous") ? paging.get("previous").getAsString() : null;
            String next = paging.has("next") ? paging.get("next").getAsString() : null;
            dayParts.setPaging(previous, next);
            if (context.hasAppSecret()) {
              dayParts.setAppSecret(context.getAppSecretProof());
            }
          }
