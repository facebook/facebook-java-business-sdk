...
public class AdCampaignDeliveryStatsUnsupportedReasons extends APINode {
  ...
  public static AdCampaignDeliveryStatsUnsupportedReasons loadJSON(String json, APIContext context, String header) {
    ...
    if (context.isDebug()) {
      ...
      JsonElement o2 = JsonParser.parseString(adCampaignDeliveryStatsUnsupportedReasons.toString());
    ...
    adCampaignDeliveryStatsUnsupportedReasons.header = header;
    return adCampaignDeliveryStatsUnsupportedReasons;
  }

  public static APINodeList<AdCampaignDeliveryStatsUnsupportedReasons> parseResponse(String json, APIContext context, APIRequest request, String header) throws MalformedResponseException {
    APINodeList<AdCampaignDeliveryStatsUnsupportedReasons> adCampaignDeliveryStatsUnsupportedReasonss = new APINodeList<AdCampaignDeliveryStatsUnsupportedReasons>(request, json, header);
    JsonArray arr;
    JsonObject obj;
    ...
    try{
      ...
      if (result.isJsonArray()) {
        ...
        for (int i = 0; i < arr.size(); i++) {
          adCampaignDeliveryStatsUnsupportedReasonss.add(loadJSON(arr.get(i).getAsJsonObject().toString(), context, header));
        };
        ...
        if (obj.has("data")) {
            ...
            for (String s : new String[]{"campaigns", "adsets", "ads"}) {
              if (obj.has(s)) {
                ...
                for (Map.Entry<String, JsonElement> entry : obj.entrySet()) {
                  adCampaignDeliveryStatsUnsupportedReasonss.add(loadJSON(entry.getValue().toString(), context, header));
                }
                break;
              }
            }
            if (!isRedownload) {
              adCampaignDeliveryStatsUnsupportedReasonss.add(loadJSON(obj.toString(), context, header));
            }
          }
          return adCampaignDeliveryStatsUnsupportedReasonss;
          ...
          for (Map.Entry entry : obj.entrySet()) {
            ...
            if (
              ...
              adCampaignDeliveryStatsUnsupportedReasonss.add(loadJSON(value.toString(), context, header));
            } else {
          ...
          adCampaignDeliveryStatsUnsupportedReasonss.add(loadJSON(json, context, header));
          return adCampaignDeliveryStatsUnsupportedReasonss;
        }
      }
    } catch (Exception e) {
  ...
  public static APIRequest.ResponseParser<AdCampaignDeliveryStatsUnsupportedReasons> getParser() {
    return new APIRequest.ResponseParser<AdCampaignDeliveryStatsUnsupportedReasons>() {
      public APINodeList<AdCampaignDeliveryStatsUnsupportedReasons> parseResponse(String response, APIContext context, APIRequest<AdCampaignDeliveryStatsUnsupportedReasons> request, String header) throws MalformedResponseException {
        return AdCampaignDeliveryStatsUnsupportedReasons.parseResponse(response, context, request, header);
      }
    };
  }
}
