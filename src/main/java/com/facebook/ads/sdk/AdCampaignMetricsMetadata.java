...
public class AdCampaignMetricsMetadata extends APINode {
  ...
  public static AdCampaignMetricsMetadata loadJSON(String json, APIContext context) {
  ...
  public static APINodeList<AdCampaignMetricsMetadata> parseResponse(String json, APIContext context, APIRequest request) throws MalformedResponseException {
    APINodeList<AdCampaignMetricsMetadata> adCampaignMetricsMetadatas = new APINodeList<AdCampaignMetricsMetadata>(request, json);
    JsonArray arr;
    JsonObject obj;
    JsonParser parser = new JsonParser();
    ...
    try{
      ...
      if (result.isJsonArray()) {
        ...
        for (int i = 0; i < arr.size(); i++) {
          adCampaignMetricsMetadatas.add(loadJSON(arr.get(i).getAsJsonObject().toString(), context));
        };
        ...
        if (obj.has("data")) {
            ...
            for (String s : new String[]{"campaigns", "adsets", "ads"}) {
              if (obj.has(s)) {
                ...
                for (Map.Entry<String, JsonElement> entry : obj.entrySet()) {
                  adCampaignMetricsMetadatas.add(loadJSON(entry.getValue().toString(), context));
                }
                break;
              }
            }
            if (!isRedownload) {
              adCampaignMetricsMetadatas.add(loadJSON(obj.toString(), context));
            }
          }
          return adCampaignMetricsMetadatas;
          ...
          for (Map.Entry entry : obj.entrySet()) {
            ...
            if (
              ...
              adCampaignMetricsMetadatas.add(loadJSON(value.toString(), context));
            } else {
          ...
          adCampaignMetricsMetadatas.add(loadJSON(json, context));
          return adCampaignMetricsMetadatas;
        }
      }
    } catch (Exception e) {
  ...
  public static APIRequest.ResponseParser<AdCampaignMetricsMetadata> getParser() {
    return new APIRequest.ResponseParser<AdCampaignMetricsMetadata>() {
      public APINodeList<AdCampaignMetricsMetadata> parseResponse(String response, APIContext context, APIRequest<AdCampaignMetricsMetadata> request) throws MalformedResponseException {
        return AdCampaignMetricsMetadata.parseResponse(response, context, request);
      }
    };
  }
}
